class VBSP {
    constructor(vbspParam) {
        vbspParam = vbspParam || {};
        var Module = typeof vbspParam !== "undefined" ? vbspParam : {};

        var key;
        var moduleOverrides = {};
        for (key in Module) if (Module.hasOwnProperty(key)) moduleOverrides[key] = Module[key];

        Module["parcCanvas"] = null;
        Module["arguments"] = [];
            ...
            function doNativeWasm(global, env, providedBuffer) {
                ...
                if (!Module["wasmBinary"] &&
                    ...
                    WebAssembly.instantiateStreaming(fetch(wasmBinaryFile), info)
                        .then((e) => { return receiveInstantiatedSource(e); })
        ...
        var GLFW = {
            ...
            onMouseWheel: (function (event) {
                var delta = -Browser.getMouseWheelDelta(event); // 当鼠标在画布上滚动时
                ...
                if (Browser.mainLoop.currentFrameNumber !== GLFW.lastGamepadStateFrame || !Browser.mainLoop.currentFrameNumber) {
                    ...
                    for (var joy = 0; joy < GLFW.lastGamepadState.length; ++joy) {
                        ...
                        if (gamepad) {
                            ...
                            if (GLFW.joys[joy]) {
                                if (GLFW.joystickFunc) Module["dynCall_vii"](GLFW.joystickFunc, joy, 262146);
        ...
        function _glfwGetMousePos(x, y) {
            return GLFW.getMousePos(GLFW.active.id, x, y);
        }
        function _glfwGetWindowSize(width, height) {
            return GLFW.getWindowSize(GLFW.active.id, width, height);
        }
        function _glfwInit() {
            if (GLFW.windows)
                return 1;
            ...
            Browser.resizeListeners.push((function (width, height) { GLFW.onCanvasResize(width, height) }));
            return 1;
        }
        ...
        function _onPointerlockMousemove(/* mouseMovementX, mouseMovementY, mouseSpeed = 1.5 */ event, mouseSpeed = 1.5) {
            // Browser.mouseMovementX = mouseMovementX;
            // Browser.mouseMovementY = mouseMovementY;
            // Browser.mouseX = Browser.mouseX + Browser.mouseMovementX * (0.1 * mouseSpeed);
            // Browser.mouseY = Browser.mouseY + Browser.mouseMovementY * (0.1 * mouseSpeed);
            ...
            if (typeof SDL != "undefined") {
                Browser.mouseX = SDL.mouseX + Browser.mouseMovementX * (0.1 * mouseSpeed);
                Browser.mouseY = SDL.mouseY + Browser.mouseMovementY * (0.1 * mouseSpeed);
            } else {
                Browser.mouseX += Browser.mouseMovementX * (0.1 * mouseSpeed);
                Browser.mouseY += Browser.mouseMovementY * (0.1 * mouseSpeed);
            }
        }
        ...
        Module.asmLibraryArg = {
            ...
            "_onPointerlockMousemove": _onPointerlockMousemove,
        ...
        Module.loadMap = (function (url) {
            if (loadedMap.includes(url)) return console.log("不允许重复下载地图");
            else loadedMap.push(url);
        ...
        function darkenDecimalColor(color, percent) {
            var amt = Math.round(2.55 * percent), R = (color >> 16) - amt, B = (color >> 8 & 0x00FF) - amt, G = (color & 0x0000FF) - amt;
            return (R < 255 ? R < 1 ? 0 : R : 255) * 0x10000 + (B < 255 ? B < 1 ? 0 : B : 255) * 0x100 + (G < 255 ? G < 1 ? 0 : G : 255);
        }
        function guess_color(name) {
            ...
            // 材质名未收录则返回默认白色（随机减弱10%色深）
            return darkenDecimalColor(16777215/*, 10 * Math.random()*/);
        }
...
if (typeof exports === 'object' && typeof module === 'object') module.exports = VBSP;
else if (typeof define === 'function' && define['amd']) define([], () => VBSP);
